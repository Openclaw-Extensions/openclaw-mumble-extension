 ...
 */
export function pcmToWav(pcm, sampleRate = MUMBLE_AUDIO_CONFIG.sampleRate, channels = MUMBLE_AUDIO_CONFIG.channels) {
 ...
 */
export function chunkAudioForEncoding(pcm) {
    const frames = [];
    ...
    for (let i = 0; i < pcm.length; i += frameSize) {
        ...
        if (chunk.length < frameSize) {
            const padded = new pcm.constructor(frameSize);
            padded.set(chunk);
            frames.push(padded);
        }
        else {
...
//# sourceMappingURL=opus-audio-pipeline.js.map