...
import { AudioCodec, } from "./mumble-audio.js";
import { MumbleOpusDecoder, MumbleOpusEncoder, AudioFrameAccumulator, pcmToWav, chunkAudioForEncoding, } from "./opus-audio-pipeline.js";
export class VoiceChatClient extends EventEmitter {
    ...
    socket;
    audioStream;
    userManager; // UserManager from mumble-client
     ...
     */
    async handleAudioPacket(packet) {
        if (!this.isInitialized) {
    ...
    async transcribeAudio(wav) {
        ...
        const response = await fetch(`${this.config.sttUrl}/audio/transcriptions`, {
            method: "POST",
            body: formData,
        });
        if (!response.ok) {
            const errorText = await response.text();
            console.error(`[voice-chat] Whisper API error: ${response.status} ${response.statusText}: ${errorText}`);
     ...
     */
    async handleAgentResponse(userText) {
    ...
    async speak(text, voice) {
        ...
        if (text !== sanitizedText) {
            console.log(`[voice-chat] Text sanitized for TTS (removed ${text.length - sanitizedText.length} chars)`);
        }
        try {
            // Generate speech via TTS service (returns WAV at 24kHz)
            const response = await fetch(`${this.config.ttsUrl}/audio/speech`, {
                method: "POST",
            ...
            const pcmData24k = new Int16Array(wavData.buffer, wavData.byteOffset + 44, (wavData.byteLength - 44) / 2);
            ...
            console.log(`[voice-chat] Sending ${frames.length} audio frames (${(frames.length * 10).toFixed(0)}ms)`);
    ...
    async sendAudioFrames(frames) {
        ...
        for (let i = 0; i < frames.length; i++) {
            try {
                // Encode frame
                const opusFrame = await this.encoder.encode(frames[i]);
                ...
                await this.audioStream.sendAudio(Buffer.from(opusFrame), AudioCodec.Opus, 0, // target: normal talking
                i === frames.length - 1);
...
//# sourceMappingURL=voice-chat-client.js.map